    ...
    return mockRequests.filter((request) => {
      ...
      if (processingDateFilter !== "all") {
        ...
        switch (processingDateFilter) {
          case "today":
            dateMatch = requestDate.toDateString() === now.toDateString();
            break;
          case "week":
          ...
          case "quarter":
            ...
            dateMatch = requestDate >= quarterAgo;
            break;
        }
      }

      // 출금 처리 관련 상태만 포함
      const isProcessingStatus = ["pending", "processing", "completed"].includes(request.status);

      return searchMatch && statusMatch && dateMatch && isProcessingStatus;
    });
  };
