...
export function NotificationCenter({ initialSubtab }: NotificationCenterProps) {
  ...
  const [activeTab, setActiveTab] = useState<"logs" | "templates" | "settings">(
    initialSubtab || "templates"
  );
  const [notificationSystem] = useState(
    () => new NotificationSystem(DEFAULT_CONFIG)
  );
  ...
  const handleTestNotification = async (templateId: string) => {
    ...
    switch (templateId) {
      case "approval_pending":
        await notificationSystem.sendApprovalPendingNotification(
          dummyRequest,
          "박CFO"
        );
        break;
      case "approval_overdue":
        await notificationSystem.sendOverdueNotification(
          dummyRequest,
          "박CFO",
          5
        );
      ...
      case "approval_rejected":
        await notificationSystem.sendApprovalRejectedNotification(
          dummyRequest,
          {
            userName: "박CFO",
      ...
      case "emergency_approval":
        await notificationSystem.sendEmergencyApprovalNotification(
          dummyRequest,
          "박CFO",
          "긴급 테스트"
        );
    ...
    switch (status) {
      case "sent":
        return {
          label: "발송 완료",
          badgeClass:
            "px-2 py-1 text-xs font-semibold rounded-full bg-sky-50 text-sky-600 border border-sky-200",
        };
      case "failed":
        return {
          label: "발송 실패",
          badgeClass:
            "px-2 py-1 text-xs font-semibold rounded-full bg-red-50 text-red-600 border border-red-200",
        };
      case "retry":
        return {
          label: "재발송 중",
          badgeClass:
            "px-2 py-1 text-xs font-semibold rounded-full bg-yellow-50 text-yellow-600 border border-yellow-200",
        };
      default:
        return {
          label: "알 수 없음",
          badgeClass:
            "px-2 py-1 text-xs font-semibold rounded-full bg-gray-50 text-gray-600 border border-gray-200",
        };
    }
  };
  ...
  const filteredLogs = logs.filter((log) => {
    ...
    const templateMatch =
      templateFilter === "all" || log.template === templateFilter;
    ...
    const searchMatch =
      ...
      (log.failureReason &&
        log.failureReason.toLowerCase().includes(logSearchTerm.toLowerCase()));
  ...
  const filteredTemplates = templates.filter((template) => {
    ...
    const matchesType =
      templateTypeFilter === "all" || template.trigger === templateTypeFilter;
  ...
  // 템플릿 페이징
  const templateTotalPages = Math.ceil(
    filteredTemplates.length / templateItemsPerPage
  );
  const templateStartIndex = (templateCurrentPage - 1) * templateItemsPerPage;
  const paginatedTemplates = filteredTemplates.slice(
    templateStartIndex,
    templateStartIndex + templateItemsPerPage
  );
  ...
  const handleSaveSettings = () => {
    // 입력 검증
    const emailValidation = Object.entries(editedEmails).every(
      ([name, email]) => {
    ...
    if (
      editedConfig.overdueThresholdHours < 1 ||
      editedConfig.overdueThresholdHours > 72
    ) {
    ...
    if (
      editedConfig.retryDelayMinutes < 1 ||
      editedConfig.retryDelayMinutes > 60
    ) {
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    <div className="sm:w-48">
                      ...
                      >
                        ...
                        <option value="approval_completed">
                          승인 완료 알림
                        </option>
                        <option value="approval_rejected">
                          승인 반려 알림
                        </option>
                          ...
                          {paginatedLogs.map((log) => (
                            <tr key={log.id}>
                              <td className="px-6 py-4 whitespace-nowrap">
                                <span
                                  className={
                                    getStatusDisplay(log.status).badgeClass
                                  }
                                >
                                  {getStatusDisplay(log.status).label}
                                </span>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      ...
                      <div className="sm:w-48">
                        <select
                          ...
                          onChange={(e) =>
                            setTemplateTypeFilter(e.target.value as any)
                          }
                          className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                        >
                      ...
                      <div className="sm:w-36">
                        <select
                          ...
                          onChange={(e) =>
                            setTemplateStatusFilter(e.target.value as any)
                          }
                          className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                        >
                              ...
                              >
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm">
                                  <div className="flex items-center justify-end space-x-2">
                                    <button
                                      onClick={() =>
                                        handleTestNotification(template.id)
                                      }
                                  ...
                                  (_, index) => {
                                    ...
                                    return (
                                      <button
                                        key={pageNumber}
                                        onClick={() =>
                                          handleTemplatePageChange(pageNumber)
                                        }
                                        className={`px-3 py-1 text-sm border rounded-md ${
                                          isCurrentPage
                                            ? "bg-primary-600 text-white border-primary-600"
                ...
                <div className="p-6 border-b border-gray-200">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 mt-1">
                        {selectedTemplate
                          ? `${selectedTemplate.name} 템플릿을 편집합니다`
                          : "새로운 알림 템플릿을 생성합니다"}
                      </p>
                    </div>
                    <button
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <select
                        defaultValue={
                          selectedTemplate?.trigger || "approval_pending"
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                      >
                        <option value="approval_pending">승인 대기</option>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        비활성화시 해당 이벤트에 대한 알림이 발송되지 않습니다
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      변수를 사용하여 동적 제목을 만들 수 있습니다. 예:{" "}
                      {`{{amount}} {{currency}}`} 출금 승인 요청
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      마크다운 문법과 변수를 사용할 수 있습니다. 사용 가능한
                      변수는 아래를 참고하세요.
                    </p>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <p className="text-xs text-gray-600 mb-4">
                      템플릿에서 아래 변수들을 사용하여 동적 내용을 만들 수
                      있습니다. 변수는 반드시{" "}
                      <code className="bg-white px-1 rounded text-primary-600">{`{{변수명}}`}</code>{" "}
                      형태로 사용해야 합니다.
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                      {[
                        ...
                        { var: "currency", desc: "통화 (BTC, ETH 등)" },
                    ...
                    <div className="mt-3 p-2 bg-blue-50 border border-blue-200 rounded text-xs">
                      <p className="text-blue-800">
                        <strong>사용 예시:</strong>{" "}
                        <code className="bg-white px-1 rounded">{`{{amount}} {{currency}}`}</code>{" "}
                        출금 신청이
                        <code className="bg-white px-1 rounded">{`{{initiator}}`}</code>
                        님에 의해 제출되었습니다.
                      </p>
                    </div>
                  </div>
                            ...
                            <div className="flex-1">
                              ...
                              {isEditingSettings ? (
                                <input
                                  ...
                                  value={email}
                                  onChange={(e) =>
                                    handleEmailChange(name, e.target.value)
                                  }
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="p-4 bg-gray-50 rounded-lg">
                    ...
                    {isEditingSettings ? (
                      ...
                      <div className="text-lg font-semibold text-gray-900">
                        {DEFAULT_CONFIG.overdueThresholdHours}시간 후
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-4 bg-gray-50 rounded-lg">
                    ...
                    {isEditingSettings ? (
                      ...
                      <div className="text-lg font-semibold text-gray-900">
                        최대 {DEFAULT_CONFIG.retryAttempts}회
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-4 bg-gray-50 rounded-lg">
                    ...
                    {isEditingSettings ? (
                      ...
                      <div className="text-lg font-semibold text-gray-900">
                        {DEFAULT_CONFIG.retryDelayMinutes}분
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="ml-3">
                  ...
                  <div className="mt-2 text-sm text-blue-700">
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <span className="font-medium">승인자명</span>: 박CFO
                      </div>
                      <div>
                        <span className="font-medium">신청자</span>: 김대리
                      </div>
                      <div>
                        <span className="font-medium">금액</span>: 1,000,000
                      </div>
                      <div>
                        <span className="font-medium">통화</span>: KRW
                      </div>
                      <div>
                        <span className="font-medium">출금주소</span>:
                        0x1234...abcd
                      </div>
                      <div>
                        <span className="font-medium">우선순위</span>: 높음
                      </div>
                      <div>
                        <span className="font-medium">지연시간</span>: 5시간
                      </div>
                      <div>
                        <span className="font-medium">반려자</span>: 이CISO
                      </div>
                      <div>
                        <span className="font-medium">반려사유</span>: 승인 권한
                        부족
                      </div>
                      <div>
                        <span className="font-medium">긴급사유</span>: 긴급 보안
                        업데이트
                      </div>
                    </div>
                  </div>
