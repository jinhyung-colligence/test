...
interface GroupApprovalAuthModalProps {
  isOpen: boolean;
  request: GroupCreationRequest | null;
  onClose: () => void;
  onAuthComplete: (sessionId: string) => void;
  onAuthFailed: (reason: string) => void;
}

export default function GroupApprovalAuthModal({
  isOpen,
  request,
  onClose,
  onAuthComplete,
  onAuthFailed,
}: GroupApprovalAuthModalProps) {
  ...
  const getBudgetDisplay = (request: GroupCreationRequest) => {
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full mx-4">
        <div className="p-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-6">
            <h4 className="font-medium text-gray-900 mb-2">{request.name} 그룹 생성</h4>
            <div className="text-sm text-gray-600">
              <p>요청 ID: #{request.id}</p>
              <p>예산: {getBudgetDisplay(request)}</p>
              <p>관리자: {request.manager}</p>
            </div>
          </div>
