...
export default function DepositManagement({ plan }: DepositManagementProps) {
  ...
  const [customERC20, setCustomERC20] = useState({ symbol: '', name: '', contractAddress: '', image: '', priceApiUrl: '' })
  ...
  const availableAssets = [
    ...
    { symbol: 'CUSTOM_ERC20', name: 'Custom ERC-20 Token', network: 'Ethereum (ERC-20)' },
  ]
  ...
  const handleAddAsset = async () => {
    ...
    if (selectedAsset === 'CUSTOM_ERC20') {
      ...
      const newAsset: Asset = {
        ...
        priceKRW: priceKRW,
        priceUSD: priceKRW ? Math.floor(priceKRW / 1300) : generateMockPrice(customERC20.symbol).usd, // Rough conversion
        priceApiUrl: customERC20.priceApiUrl
      }

      setAssets([...assets, newAsset])
      setCustomERC20({ symbol: '', name: '', contractAddress: '', image: '', priceApiUrl: '' })
      setImagePreview('')
  ...
  const generateMockPrice = (symbol: string): { krw: number, usd: number } => {
    ...
    const mockPrices: { [key: string]: { krw: number, usd: number } } = {
      'BTC': { krw: Math.floor(Math.random() * 10000000 + 60000000), usd: Math.floor(Math.random() * 5000 + 45000) },
      'ETH': { krw: Math.floor(Math.random() * 1000000 + 3000000), usd: Math.floor(Math.random() * 500 + 2300) },
      'SOL': { krw: Math.floor(Math.random() * 50000 + 100000), usd: Math.floor(Math.random() * 50 + 75) },
      'USDT': { krw: Math.floor(Math.random() * 50 + 1300), usd: 1 },
      'USDC': { krw: Math.floor(Math.random() * 50 + 1300), usd: 1 },
      'MATIC': { krw: Math.floor(Math.random() * 200 + 800), usd: Math.floor(Math.random() * 20 + 60) / 100 },
      'ADA': { krw: Math.floor(Math.random() * 200 + 500), usd: Math.floor(Math.random() * 30 + 35) / 100 },
      'DOT': { krw: Math.floor(Math.random() * 2000 + 8000), usd: Math.floor(Math.random() * 200 + 600) / 100 },
      'XRP': { krw: Math.floor(Math.random() * 200 + 700), usd: Math.floor(Math.random() * 40 + 50) / 100 },
    }
  ...
  const generateMockTransactions = (assetSymbol: string): Transaction[] => {
    ...
    const statuses: ('confirmed' | 'pending' | 'failed')[] = ['confirmed', 'confirmed', 'confirmed', 'pending', 'confirmed']
    
    for (let i = 0; i < 5; i++) {
      const status = statuses[i]
      const timestamp = new Date(Date.now() - (i * 24 * 60 * 60 * 1000 + Math.random() * 12 * 60 * 60 * 1000))
      ...
      transactions.push({
        ...
        status,
        confirmations: status === 'confirmed' ? Math.floor(Math.random() * 100) + 6 : Math.floor(Math.random() * 3),
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">암호화폐 자산 입금을 위한 주소를 관리합니다</p>
        </div>
        <button
            ...
            <div className="space-y-4">
                ...
                >
                  ...
                  {filteredAvailableAssets.map((asset) => (
                    <option key={asset.symbol} value={asset.symbol}>
                      {asset.symbol === 'CUSTOM_ERC20' ? '커스텀 ERC-20 토큰' : `${asset.symbol} - ${asset.name} (${asset.network})`}
                    </option>
                  ))}
                </select>
              </div>
              ...
              {showCustomERC20 && (
                <div className="space-y-4 p-4 border border-gray-200 rounded-lg">
                  <h4 className="text-sm font-semibold text-gray-900">ERC-20 토큰 정보 입력</h4>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setCustomERC20(prev => ({ ...prev, contractAddress: e.target.value }))}
                      placeholder="0x..."
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setCustomERC20(prev => ({ ...prev, name: e.target.value }))}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setCustomERC20(prev => ({ ...prev, symbol: e.target.value }))}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setCustomERC20(prev => ({ ...prev, priceApiUrl: e.target.value }))}
              ...
              <div className="flex space-x-3 pt-4">
                ...
                <button
                  ...
                  disabled={!selectedAsset || (selectedAsset === 'CUSTOM_ERC20' && (!customERC20.symbol || !customERC20.name || !customERC20.contractAddress))}
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-4xl max-h-[80vh] overflow-hidden">
            ...
            {showStatusHelp && (
              <div className="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                <h4 className="text-sm font-semibold text-blue-900 mb-3">상태 및 확인수 설명</h4>
                <div className="space-y-3 text-sm">
                  <div>
                    <span className="font-medium text-blue-900">상태:</span>
                    <ul className="mt-1 space-y-1 ml-4">
                      <li className="flex items-center space-x-2">
                        <span className="px-2 py-0.5 text-xs font-medium rounded-full border text-green-600 bg-green-50 border-green-200">완료</span>
                        <span className="text-gray-700">트랜잭션이 성공적으로 처리됨</span>
                      </li>
                      <li className="flex items-center space-x-2">
                        <span className="px-2 py-0.5 text-xs font-medium rounded-full border text-yellow-600 bg-yellow-50 border-yellow-200">대기</span>
                        <span className="text-gray-700">블록체인에서 처리 중</span>
                      </li>
                      <li className="flex items-center space-x-2">
                        <span className="px-2 py-0.5 text-xs font-medium rounded-full border text-red-600 bg-red-50 border-red-200">실패</span>
                        <span className="text-gray-700">트랜잭션 처리 실패</span>
                      </li>
                    </ul>
                  </div>
                  <div>
                    ...
                    <ul className="mt-1 space-y-1 ml-4 text-gray-700">
                      <li><strong>예시 101/6+</strong> = 현재 101번 확인됨 / 최소 6번 필요</li>
                      <li>확인수가 많을수록 트랜잭션이 더 안전하게 확정됨</li>
                      <li><strong>BTC:</strong> 6 확인, <strong>ETH/USDT/USDC:</strong> 12 확인, <strong>SOL:</strong> 32 확인 권장</li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {generateMockTransactions(selectedAssetHistory.symbol).map((tx) => (
                      <tr key={tx.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3">
                          <div className="text-xs font-mono text-gray-900 truncate max-w-32" title={tx.txHash}>
                            {tx.txHash.length > 16 ? `${tx.txHash.substring(0, 8)}...${tx.txHash.substring(tx.txHash.length - 8)}` : tx.txHash}
                          </div>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <div className="text-xs font-mono text-gray-600 truncate max-w-24" title={tx.fromAddress}>
                            {tx.fromAddress.length > 12 ? `${tx.fromAddress.substring(0, 6)}...${tx.fromAddress.substring(tx.fromAddress.length - 6)}` : tx.fromAddress}
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`px-2 py-1 text-xs font-medium rounded-full border ${getStatusColor(tx.status)}`}>
                            {getStatusText(tx.status)}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="text-sm text-gray-900">
                            {tx.confirmations}{tx.status === 'confirmed' ? '/6+' : '/6'}
                          </div>
                        </td>
                        <td className="px-4 py-3">
                    ...
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        {asset.contractAddress && (
                          <div className="text-xs font-mono text-gray-400 truncate max-w-32" title={asset.contractAddress}>
                            {asset.contractAddress.substring(0, 8)}...{asset.contractAddress.substring(asset.contractAddress.length - 6)}
                          </div>
                        )}
                      </div>
                    </div>
      ...
      {assets.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">자산이 없습니다</h3>
          <p className="text-gray-600 mb-4">첫 번째 자산을 추가하여 입금을 시작하세요</p>
          <button
            onClick={() => setShowAddAsset(true)}
